...
const runPdfToImages = async (inputPath: string, outputDir: string, base: string) => {
...
const processJob = async (jobId: string) => {
  ...
  const handleOcrDocx = async (input: JobInput & { baseName: string; localPath: string }) => {
  ...
  const handleOcrXlsx = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputBase = path.join(outputDir, sanitizeFileName(input.baseName));
    const textPath = await runTesseractToText(input.localPath, outputBase);
    ...
    const rows = text
      ...
      .join("\n");
    const csvPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.csv`);
  ...
  const handleImageTranslate = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputBase = path.join(outputDir, sanitizeFileName(input.baseName));
    const textPath = await runTesseractToText(input.localPath, outputBase);
    const text = await readFile(textPath, "utf8");
    const translated = await translateText(text);
    const translatedPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.txt`);
    await writeFile(translatedPath, translated, "utf8");
    await addOutput(translatedPath, buildOutputName(input.baseName, "txt"));
  };

  const handlePdfToText = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.txt`);
  ...
  const handlePdfTables = async (
    ...
    const outputPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.${format}`);
    await runPdfTableExtract(input.localPath, outputPath);
    await addOutput(outputPath, buildOutputName(input.baseName, format));
  };

  const handlePdfToJpg = async (input: JobInput & { baseName: string; localPath: string }) => {
    const pageDir = path.join(outputDir, `${sanitizeFileName(input.baseName)}-pages`);
    await ensureDir(pageDir);
    const images = await runPdfToImages(input.localPath, pageDir, "page");
    if (images.length === 0) {
      throw new Error("No pages rendered.");
    }
    const zipPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}-pages.zip`);
    await zipFiles(zipPath, images);
    await addOutput(zipPath, `${sanitizeFileName(input.baseName)}-pages.zip`);
  };

  const handleOfficeToJpg = async (input: JobInput & { baseName: string; localPath: string }) => {
    const pdfPath = await convertWithLibreOffice(input.localPath, "pdf", outputDir);
    const pageDir = path.join(outputDir, `${sanitizeFileName(input.baseName)}-pages`);
    await ensureDir(pageDir);
  ...
  const handlePdfToHtml = async (input: JobInput & { baseName: string; localPath: string }) => {
    const htmlDir = path.join(outputDir, `${sanitizeFileName(input.baseName)}-html`);
    await ensureDir(htmlDir);
    await runPdfToHtml(input.localPath, htmlDir);
    const zipPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}-html.zip`);
  ...
  const handleSvgToPng = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.png`);
  ...
  const handleImageToPdf = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.pdf`);
    await runImageToPdf(input.localPath, outputPath);
    await addOutput(outputPath, buildOutputName(input.baseName, "pdf"));
  };

  const handleQrScan = async (input: JobInput & { baseName: string; localPath: string }) => {
  ...
  const handleQrGenerate = async (input: JobInput & { baseName: string; localPath: string }) => {
  ...
  const handleCsvToJson = async (input: JobInput & { baseName: string; localPath: string }) => {
    const outputPath = path.join(outputDir, `${sanitizeFileName(input.baseName)}.json`);
